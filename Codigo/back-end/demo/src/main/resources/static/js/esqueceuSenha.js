async function resetPassword() {
  ...
  try {
    ...
    const response = await fetch('https://historical-pepi-puc-tis-d0d4bf2c.koyeb.app/api/usuarios/esqueceu-senha', {
      method: 'POST',
...
async function solicitarToken(email) {
  try {
    const response = await fetch(`https://historical-pepi-puc-tis-d0d4bf2c.koyeb.app/api/usuarios/solicitar?userEmail=${encodeURIComponent(email)}`, {
      method: 'POST'
    });
...
async function validarToken() {
  ...
  try {
    const response = await fetch(`https://historical-pepi-puc-tis-d0d4bf2c.koyeb.app/api/usuarios/validar?userEmail=${encodeURIComponent(email)}&token=${encodeURIComponent(token)}`, {
      method: 'POST'
    });
